...
import "../../src/dialects/dremio.js";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/** Round-trip: parse as Dremio, generate as Dremio, compare. */
function validateIdentity(sql: string, writeSql?: string) {
  const result = transpile(sql, { readDialect: "dremio", writeDialect: "dremio" })[0];
...
function validateTranspile(
  ...
  const result = transpile(sql, {
    readDialect: opts.read ?? "dremio",
    writeDialect: opts.write ?? "dremio",
  })[0];
...
describe("Dremio dialect", () => {
  ...
  it("should be registered", () => {
    const d = Dialect.getOrRaise("dremio");
  ...
  describe("cross-dialect transpilation", () => {
    ...
    it("dremio -> postgres (identifier quoting stays double-quote)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("postgres -> dremio (identifier quoting stays double-quote)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("dremio -> mysql (double-quote to backtick)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("mysql -> dremio (backtick to double-quote)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("dremio -> databricks (double-quote to backtick)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("databricks -> dremio (backtick to double-quote)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("dremio -> duckdb (double-quote preserved)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
  ...
  describe("CAST and type mappings", () => {
    ...
    // Type mappings: standard SQL types -> Dremio equivalents
    it("TEXT -> VARCHAR", () => {
      validateTranspile(
  ...
  // -------------------------------------------------------------------------
  // ILIKE (NOT supported in Dremio)
  // -------------------------------------------------------------------------

  describe("ILIKE", () => {
    it.todo("ILIKE is not natively supported in Dremio - should transpile to LIKE with UPPER/LOWER");
  });
  ...
  describe("cross-dialect type transpilation", () => {
    it("postgres VARCHAR -> dremio VARCHAR (preserved)", () => {
      validateTranspile(
    ...
    it("postgres TEXT -> dremio VARCHAR", () => {
      validateTranspile(
        "SELECT CAST(a AS TEXT)",
        { read: "postgres", expected: "SELECT CAST(a AS VARCHAR)" },
      );
    ...
    it.todo("databricks STRING -> dremio VARCHAR (requires AST type normalization)");

    it("dremio BOOLEAN -> bigquery BOOL", () => {
      validateTranspile(
    ...
    it("dremio INT -> clickhouse Int32", () => {
      validateTranspile(
        "SELECT CAST(a AS INT)",
    ...
    it("snowflake VARIANT -> dremio (identity fallback)", () => {
      validateTranspile(
        "SELECT CAST(a AS VARIANT)",
        { read: "snowflake", expected: "SELECT CAST(a AS VARIANT)" },
      );
    });
  });
  ...
  describe("todo: advanced features", () => {
    it.todo("ILIKE transpilation to UPPER/LOWER + LIKE");
    it.todo("TRY_CAST -> CAST conversion (Dremio has no TRY_CAST)");
    it.todo("DATE_ADD / DATE_SUB with INTERVAL CAST");
    it.todo("TO_CHAR / TO_DATE format strings");
    it.todo("ARRAY_GENERATE_RANGE -> GenerateSeries");
    it.todo("BIT_AND / BIT_OR aggregate functions");
    it.todo("REPEATSTR -> REPEAT");
    it.todo("REGEXP_MATCHES -> REGEXP_LIKE");
    it.todo("CURRENT_DATE_UTC function");
    it.todo("// single-line comment style");
    it.todo("TIMESTAMPTZ/TIMESTAMPLTZ unsupported type warning");
    it.todo("LIST type (ARRAY -> LIST mapping)");
    it.todo("PIVOT / UNPIVOT");
    it.todo("MERGE INTO");
    it.todo("CREATE TABLE AS SELECT");
    it.todo("DESCRIBE TABLE");
    it.todo("SHOW TABLES / SCHEMAS");
    it.todo("ALTER TABLE");
    it.todo("FLATTEN / UNNEST");
  });
});
