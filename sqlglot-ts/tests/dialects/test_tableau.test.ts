...
import "../../src/dialects/tableau.js";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/** Round-trip: parse as Tableau, generate as Tableau, compare. */
function validateIdentity(sql: string, writeSql?: string) {
  const result = transpile(sql, { readDialect: "tableau", writeDialect: "tableau" })[0];
...
function validateTranspile(
  ...
  const result = transpile(sql, {
    readDialect: opts.read ?? "tableau",
    writeDialect: opts.write ?? "tableau",
  })[0];
...
describe("Tableau dialect", () => {
  // -------------------------------------------------------------------------
  // Dialect registration
  // -------------------------------------------------------------------------

  it("should be registered", () => {
    const d = Dialect.getOrRaise("tableau");
    expect(d).toBeDefined();
  });

  it("constructor name is Tableau", () => {
    const d = Dialect.getOrRaise("tableau");
    expect(d.constructor.name).toBe("Tableau");
  });
  ...
  describe("cross-dialect transpilation", () => {
    ...
    it("tableau -> postgres (same quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("postgres -> tableau (same quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("tableau -> mysql (identifier quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("mysql -> tableau (identifier quoting)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
  ...
  describe("CAST and type mappings", () => {
    ...
    // Type mappings from standard SQL to Tableau
    it("TEXT -> VARCHAR", () => {
      validateTranspile(
  ...
  // -------------------------------------------------------------------------
  // ILIKE support (Tableau does not support ILIKE natively)
  // -------------------------------------------------------------------------

  describe("ILIKE", () => {
    it("ILIKE round-trips as ILIKE (base behavior)", () => {
      // Tableau does not have special ILIKE handling, so it uses base dialect behavior
  ...
  describe("cross-dialect type transpilation", () => {
    it("postgres VARCHAR -> tableau VARCHAR", () => {
      validateTranspile(
    ...
    it("postgres TEXT -> tableau VARCHAR", () => {
      validateTranspile(
        "SELECT CAST(a AS TEXT)",
        { read: "postgres", expected: "SELECT CAST(a AS VARCHAR)" },
      );
    ...
    it.todo("databricks STRING -> tableau VARCHAR (cross-dialect type normalization)");

    it("tableau BOOLEAN -> bigquery BOOL", () => {
      validateTranspile(
    ...
    it("tableau DOUBLE -> postgres DOUBLE PRECISION", () => {
      validateTranspile(
        "SELECT CAST(a AS DOUBLE)",
        { write: "postgres", expected: "SELECT CAST(a AS DOUBLE PRECISION)" },
      );
    });
  });
  ...
  describe("todo: advanced features", () => {
    it.todo("IF THEN ELSE END syntax");
    it.todo("COUNTD function (Tableau-specific COUNT DISTINCT)");
    it.todo("FIND / FINDNTH functions");
    it.todo("IFNULL -> COALESCE transpilation");
    it.todo("Tableau calculated fields");
    it.todo("LOD expressions (FIXED, INCLUDE, EXCLUDE)");
    it.todo("ZN function");
    it.todo("DATETRUNC function");
    it.todo("DATEPART function");
    it.todo("DATEDIFF function");
    it.todo("CONTAINS function");
    it.todo("LIMIT/OFFSET");
    it.todo("Square bracket identifier quoting [col]");
  });
});
