...
/** Round-trip: parse as Databricks, generate as Databricks, compare. */
function validateIdentity(sql: string, writeSql?: string) {
  const result = transpile(sql, { readDialect: "databricks", writeDialect: "databricks" })[0];
...
function validateTranspile(
  ...
  const result = transpile(sql, {
    readDialect: opts.read ?? "databricks",
    writeDialect: opts.write ?? "databricks",
  })[0];
...
describe("Databricks dialect", () => {
  ...
  it("should be registered", () => {
    const d = Dialect.getOrRaise("databricks");
  ...
  describe("cross-dialect transpilation", () => {
    ...
    it("databricks → postgres (identifier quoting)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("postgres → databricks (identifier quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("databricks → mysql (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("mysql → databricks (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
  ...
  describe("CAST and type mappings", () => {
    ...
    // Type mappings from standard SQL to Databricks
    it("VARCHAR → STRING", () => {
      validateTranspile(
  ...
  describe("comments", () => {
    it("single-line comment (--) parses", () => {
      ...
      const result = transpile("SELECT a -- comment\nFROM t", {
        readDialect: "databricks",
        writeDialect: "databricks",
      })[0];
  ...
  describe("cross-dialect type transpilation", () => {
    it("postgres VARCHAR → databricks STRING", () => {
      validateTranspile(
        'SELECT CAST(a AS VARCHAR)',
    ...
    it("postgres TEXT → databricks STRING", () => {
      validateTranspile(
        'SELECT CAST(a AS TEXT)',
        { read: "postgres", expected: "SELECT CAST(a AS STRING)" },
      );
    ...
    it("databricks INT → postgres INT", () => {
      validateTranspile(
    ...
    it("mysql → databricks (BLOB → BINARY)", () => {
      validateTranspile(
        "SELECT CAST(a AS BLOB)",
    ...
    it("databricks BOOLEAN → bigquery BOOL", () => {
      validateTranspile(
    ...
    it("databricks → clickhouse (type mappings)", () => {
      validateTranspile(
        "SELECT CAST(a AS INT)",
  ...
  describe("todo: advanced features", () => {
    ...
    it.todo("MERGE INTO");
    it.todo("DELTA LAKE: DESCRIBE HISTORY");
    it.todo("DELTA LAKE: OPTIMIZE");
    it.todo("DELTA LAKE: VACUUM");
    it.todo("CLONE TABLE");
    ...
    it.todo("JSON colon extraction syntax (col:path)");
    it.todo("QUALIFY clause");
    it.todo("IDENTIFIER function");
