...
/** Round-trip: parse as Spark2, generate as Spark2, compare. */
function validateIdentity(sql: string, writeSql?: string) {
  const result = transpile(sql, { readDialect: "spark2", writeDialect: "spark2" })[0];
...
function validateTranspile(
  ...
  const result = transpile(sql, {
    readDialect: opts.read ?? "spark2",
    writeDialect: opts.write ?? "spark2",
  })[0];
...
describe("Spark2 dialect", () => {
  // -------------------------------------------------------------------------
  // Dialect registration
  // -------------------------------------------------------------------------

  it("should be registered", () => {
    const d = Dialect.getOrRaise("spark2");
  ...
  describe("cross-dialect transpilation", () => {
    ...
    it("spark2 -> postgres (identifier quoting)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("postgres -> spark2 (identifier quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("spark2 -> mysql (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("mysql -> spark2 (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
  ...
  describe("GROUP BY / HAVING / ORDER BY", () => {
    ...
    it("ORDER BY multiple columns (ASC implicit)", () => {
  ...
  describe("CAST and type mappings", () => {
    // Native Spark2 types that round-trip
    it("CAST to INT", () => {
    ...
    // Type mappings from standard SQL to Spark2
    it("VARCHAR -> STRING", () => {
      validateTranspile(
  ...
  describe("ILIKE", () => {
    it.todo("ILIKE round-trips natively (Spark2 does NOT support ILIKE in all versions)");

    it.todo("ILIKE with NOT");
  });
  ...
  describe("cross-dialect type transpilation", () => {
    it("postgres VARCHAR -> spark2 STRING", () => {
      validateTranspile(
        "SELECT CAST(a AS VARCHAR)",
    ...
    it("postgres TEXT -> spark2 STRING", () => {
      validateTranspile(
        "SELECT CAST(a AS TEXT)",
        { read: "postgres", expected: "SELECT CAST(a AS STRING)" },
      );
    ...
    it("spark2 BOOLEAN -> bigquery BOOL", () => {
      validateTranspile(
    ...
    it("spark2 -> clickhouse (type mappings)", () => {
      validateTranspile(
        "SELECT CAST(a AS INT)",
  ...
  describe("todo: advanced features", () => {
    ...
    it.todo("HEX string literals (X'...')");
    it.todo("APPROX_COUNT_DISTINCT");
    it.todo("ARRAY_JOIN");
    it.todo("MAP_FROM_ARRAYS");
    it.todo("SHIFTLEFT / SHIFTRIGHT");
    it.todo("DATE_TRUNC / TRUNC");
    it.todo("TO_TIMESTAMP / TO_DATE");
    it.todo("FROM_UTC_TIMESTAMP / TO_UTC_TIMESTAMP");
    it.todo("AGGREGATE / REDUCE");
  });
});
