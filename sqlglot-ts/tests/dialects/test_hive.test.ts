...
/** Round-trip: parse as Hive, generate as Hive, compare. */
function validateIdentity(sql: string, writeSql?: string) {
  const result = transpile(sql, { readDialect: "hive", writeDialect: "hive" })[0];
...
function validateTranspile(
  ...
  const result = transpile(sql, {
    readDialect: opts.read ?? "hive",
    writeDialect: opts.write ?? "hive",
  })[0];
...
describe("Hive dialect", () => {
  // -------------------------------------------------------------------------
  // Dialect registration
  // -------------------------------------------------------------------------

  it("should be registered", () => {
    const d = Dialect.getOrRaise("hive");
  ...
  describe("cross-dialect transpilation", () => {
    ...
    it("hive -> postgres (identifier quoting)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("postgres -> hive (identifier quoting)", () => {
      validateTranspile(
        'SELECT "col" FROM "t"',
    ...
    it("hive -> mysql (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("mysql -> hive (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("hive -> databricks (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
    ...
    it("databricks -> hive (backtick to backtick)", () => {
      validateTranspile(
        "SELECT `col` FROM `t`",
  ...
  describe("CAST and type mappings", () => {
    ...
    // Type mappings from standard SQL to Hive
    it("VARCHAR -> STRING", () => {
      validateTranspile(
  ...
  // -------------------------------------------------------------------------
  // Hive type aliases via tokenizer
  // -------------------------------------------------------------------------

  describe("Hive type aliases", () => {
  ...
  // -------------------------------------------------------------------------
  // ILIKE support (Hive does NOT support ILIKE natively)
  // -------------------------------------------------------------------------
  ...
  describe("ILIKE", () => {
    ...
    it("ILIKE from postgres converted to LIKE in hive", () => {
      validateTranspile(
        "SELECT * FROM t WHERE name ILIKE '%test%'",
  ...
  describe("aggregate functions", () => {
    ...
    it.todo("COUNT DISTINCT (generates with comma)");
  });
  ...
  describe("cross-dialect type transpilation", () => {
    it("postgres VARCHAR -> hive STRING", () => {
      validateTranspile(
        "SELECT CAST(a AS VARCHAR)",
    ...
    it("hive -> clickhouse (type mappings)", () => {
      validateTranspile(
        "SELECT CAST(a AS INT)",
    ...
    it.todo("hive STRING -> postgres TEXT (requires parser type normalization)");

    it("hive -> databricks (type preserved)", () => {
      validateTranspile(
  ...
  describe("todo: advanced features", () => {
    ...
    it.todo("SORT BY");
    it.todo("TABLESAMPLE");
    it.todo("TRANSFORM / MAP / REDUCE");
    it.todo("CREATE TABLE");
    it.todo("CREATE TABLE AS SELECT");
    it.todo("PARTITIONED BY");
    it.todo("ROW FORMAT SERDE");
    it.todo("STORED AS");
    it.todo("SERDEPROPERTIES");
    it.todo("MSCK REPAIR TABLE");
    it.todo("ADD JAR / FILE");
    ...
    it.todo("COLLECT_LIST / COLLECT_SET");
    it.todo("PERCENTILE / PERCENTILE_APPROX");
    it.todo("GET_JSON_OBJECT");
    it.todo("REGEXP_EXTRACT / REGEXP_REPLACE");
    it.todo("DATE_ADD / DATE_SUB / DATEDIFF");
    it.todo("FROM_UNIXTIME / UNIX_TIMESTAMP");
    it.todo("TO_DATE / DATE_FORMAT");
    it.todo("EXPLODE / POSEXPLODE");
    it.todo("named parameter syntax (${param})");
    it.todo("double-quote string parsing");
    it.todo("numeric literal suffixes (L, S, Y, D, F, BD)");
  });
});
