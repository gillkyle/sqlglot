import { describe, it, expect } from "vitest";
import { transpile, Dialect } from "../../src/index.js";

/**
 * Validate that SQL round-trips through parse -> generate using the ClickHouse dialect.
 */
function validateIdentity(sql: string, writeSql?: string): void {
  const result = transpile(sql, { readDialect: "clickhouse", writeDialect: "clickhouse" })[0];
  expect(result).toBe(writeSql ?? sql);
}

/**
 * Validate transpilation from one dialect to another.
 */
function validateTranspile(
  sql: string,
  opts: {
    read?: string;
    write?: string;
    expected: string;
  },
): void {
  const result = transpile(sql, {
    readDialect: opts.read ?? "clickhouse",
    writeDialect: opts.write ?? "clickhouse",
  })[0];
  expect(result).toBe(opts.expected);
}

// =============================================================================
// ClickHouse dialect registration
// =============================================================================

describe("ClickHouse: dialect registration", () => {
  it("registers under 'clickhouse'", () => {
    const d = Dialect.getOrRaise("clickhouse");
    expect(d).toBeDefined();
    expect(d.constructor.name).toBe("ClickHouse");
  });

  it("uses backtick for identifiers", () => {
    const d = Dialect.getOrRaise("clickhouse");
    expect(d.IDENTIFIER_START).toBe("`");
    expect(d.IDENTIFIER_END).toBe("`");
  });

  it("has case-sensitive function names (NORMALIZE_FUNCTIONS = false)", () => {
    const d = Dialect.getOrRaise("clickhouse");
    expect(d.NORMALIZE_FUNCTIONS).toBe(false);
  });
});

// =============================================================================
// ClickHouse Identifier Quoting Tests
// =============================================================================

describe("ClickHouse: identifier quoting with backticks", () => {
  it("backtick-quoted identifiers round-trip", () => {
    validateIdentity("SELECT `a` FROM `b`");
  });

  it("backtick-quoted table.column round-trip", () => {
    validateIdentity("SELECT `a`.`b` FROM `a`");
  });

  it("mixed quoted and unquoted identifiers", () => {
    validateIdentity("SELECT `a`.b FROM a");
  });

  it("backtick-quoted alias", () => {
    validateIdentity("SELECT 1 AS `my alias`");
  });

  it("backtick three-part name", () => {
    validateIdentity("SELECT `a`.`b`.`c`");
  });

  it("unquoted identifiers remain unquoted", () => {
    validateIdentity("SELECT a FROM b");
  });

  it("double-quoted identifiers from read become backtick-quoted", () => {
    const result = transpile('SELECT "a" FROM "b"', {
      readDialect: "clickhouse",
      writeDialect: "clickhouse",
    })[0];
    // ClickHouse tokenizer accepts double-quotes as identifiers,
    // output uses the dialect's default backtick quoting
    expect(result).toBe("SELECT `a` FROM `b`");
  });
});

// =============================================================================
// ClickHouse to/from other dialects transpilation
// =============================================================================

describe("ClickHouse: transpile to default dialect", () => {
  it("backtick identifiers become double-quoted in default dialect", () => {
    validateTranspile("SELECT `a` FROM `b`", {
      read: "clickhouse",
      write: "",
      expected: 'SELECT "a" FROM "b"',
    });
  });

  it("unquoted identifiers pass through", () => {
    validateTranspile("SELECT a FROM b", {
      read: "clickhouse",
      write: "",
      expected: "SELECT a FROM b",
    });
  });
});

describe("ClickHouse: transpile from default dialect", () => {
  it("double-quoted identifiers become backtick-quoted in ClickHouse", () => {
    validateTranspile('SELECT "a" FROM "b"', {
      read: "",
      write: "clickhouse",
      expected: "SELECT `a` FROM `b`",
    });
  });
});

describe("ClickHouse: transpile to/from MySQL", () => {
  it("MySQL backticks stay backticks in ClickHouse", () => {
    validateTranspile("SELECT `col` FROM `tbl`", {
      read: "mysql",
      write: "clickhouse",
      expected: "SELECT `col` FROM `tbl`",
    });
  });

  it("ClickHouse backticks stay backticks in MySQL", () => {
    validateTranspile("SELECT `col` FROM `tbl`", {
      read: "clickhouse",
      write: "mysql",
      expected: "SELECT `col` FROM `tbl`",
    });
  });
});

describe("ClickHouse: transpile to/from Postgres", () => {
  it("Postgres double-quotes become backticks in ClickHouse", () => {
    validateTranspile('SELECT "col" FROM "tbl"', {
      read: "postgres",
      write: "clickhouse",
      expected: "SELECT `col` FROM `tbl`",
    });
  });

  it("ClickHouse backticks become double-quotes in Postgres", () => {
    validateTranspile("SELECT `col` FROM `tbl`", {
      read: "clickhouse",
      write: "postgres",
      expected: 'SELECT "col" FROM "tbl"',
    });
  });
});

describe("ClickHouse: transpile to/from DuckDB", () => {
  it("DuckDB double-quotes become backticks in ClickHouse", () => {
    validateTranspile('SELECT "col" FROM "tbl"', {
      read: "duckdb",
      write: "clickhouse",
      expected: "SELECT `col` FROM `tbl`",
    });
  });

  it("ClickHouse backticks become double-quotes in DuckDB", () => {
    validateTranspile("SELECT `col` FROM `tbl`", {
      read: "clickhouse",
      write: "duckdb",
      expected: 'SELECT "col" FROM "tbl"',
    });
  });
});

// =============================================================================
// ClickHouse SELECT basics
// =============================================================================

describe("ClickHouse: SELECT basics", () => {
  it("SELECT * FROM test", () => {
    validateIdentity("SELECT * FROM test");
  });

  it("SELECT 1", () => {
    validateIdentity("SELECT 1");
  });

  it("SELECT with alias", () => {
    validateIdentity("SELECT a AS b FROM test");
  });

  it("SELECT with string literal", () => {
    validateIdentity("SELECT 'hello'");
  });

  it("SELECT with number", () => {
    validateIdentity("SELECT 42");
  });

  it("SELECT with multiple columns", () => {
    validateIdentity("SELECT a, b, c FROM test");
  });

  it("SELECT with expressions", () => {
    validateIdentity("SELECT a + b AS total FROM test");
  });

  it("SELECT with arithmetic", () => {
    validateIdentity("SELECT 1 + 2 * 3");
  });

  it("SELECT with comparison", () => {
    validateIdentity("SELECT a > 1 FROM test");
  });
});

// =============================================================================
// ClickHouse WHERE clause
// =============================================================================

describe("ClickHouse: WHERE clause", () => {
  it("simple WHERE", () => {
    validateIdentity("SELECT a FROM test WHERE a = 1");
  });

  it("WHERE with AND", () => {
    validateIdentity("SELECT a FROM test WHERE a = 1 AND b = 2");
  });

  it("WHERE with OR", () => {
    validateIdentity("SELECT a FROM test WHERE a = 1 OR b = 2");
  });

  it("WHERE with IN", () => {
    validateIdentity("SELECT a FROM test WHERE a IN (1, 2, 3)");
  });

  it("WHERE with BETWEEN", () => {
    validateIdentity("SELECT a FROM test WHERE a BETWEEN 1 AND 10");
  });

  it("WHERE with LIKE", () => {
    validateIdentity("SELECT a FROM test WHERE a LIKE '%test%'");
  });

  it("WHERE with ILIKE (ClickHouse supports ILIKE)", () => {
    validateIdentity("SELECT a FROM test WHERE a ILIKE '%test%'");
  });

  it("WHERE with NOT", () => {
    validateIdentity("SELECT a FROM test WHERE NOT a = 1");
  });

  it("WHERE with IS NULL", () => {
    validateIdentity("SELECT a FROM test WHERE a IS NULL");
  });

  it("WHERE with subquery IN", () => {
    validateIdentity("SELECT a FROM test WHERE a IN (SELECT b FROM other)");
  });
});

// =============================================================================
// ClickHouse GROUP BY, HAVING, ORDER BY
// =============================================================================

describe("ClickHouse: GROUP BY, HAVING, ORDER BY", () => {
  it("GROUP BY", () => {
    validateIdentity("SELECT a, COUNT(*) FROM test GROUP BY a");
  });

  it("GROUP BY with HAVING", () => {
    validateIdentity(
      "SELECT a, COUNT(*) FROM test GROUP BY a HAVING COUNT(*) > 1",
    );
  });

  it("ORDER BY", () => {
    validateIdentity("SELECT a FROM test ORDER BY a");
  });

  it("ORDER BY DESC", () => {
    validateIdentity("SELECT a FROM test ORDER BY a DESC");
  });

  it("ORDER BY multiple", () => {
    validateIdentity("SELECT a, b FROM test ORDER BY a, b DESC");
  });

  it("ORDER BY NULLS FIRST", () => {
    validateIdentity("SELECT a FROM test ORDER BY a NULLS FIRST");
  });

  it("ORDER BY NULLS LAST", () => {
    validateIdentity("SELECT a FROM test ORDER BY a NULLS LAST");
  });
});

// =============================================================================
// ClickHouse JOINs
// =============================================================================

describe("ClickHouse: JOINs", () => {
  it("JOIN ... ON", () => {
    validateIdentity("SELECT 1 FROM a JOIN b ON a.x = b.x");
  });

  it("INNER JOIN", () => {
    validateIdentity("SELECT 1 FROM a INNER JOIN b ON a.x = b.x");
  });

  it("CROSS JOIN", () => {
    validateIdentity("SELECT 1 FROM a CROSS JOIN b ON a.x = b.x");
  });

  it("JOIN ... USING", () => {
    validateIdentity("SELECT 1 FROM a JOIN b USING (x)");
  });

  it("multiple JOINs", () => {
    validateIdentity(
      "SELECT 1 FROM a JOIN b ON a.x = b.x JOIN c ON b.y = c.y",
    );
  });

  it("comma join", () => {
    validateIdentity("SELECT * FROM a, b");
  });
});

// =============================================================================
// ClickHouse Set Operations
// =============================================================================

describe("ClickHouse: set operations", () => {
  it("UNION", () => {
    validateIdentity("SELECT 1 UNION SELECT 2");
  });

  it("UNION ALL", () => {
    validateIdentity("SELECT 1 UNION ALL SELECT 2");
  });

  it("EXCEPT", () => {
    validateIdentity("SELECT 1 EXCEPT SELECT 2");
  });

  it("INTERSECT", () => {
    validateIdentity("SELECT 1 INTERSECT SELECT 2");
  });
});

// =============================================================================
// ClickHouse CTEs
// =============================================================================

describe("ClickHouse: CTEs", () => {
  it("simple CTE", () => {
    validateIdentity("WITH cte AS (SELECT 1) SELECT * FROM cte");
  });

  it("multiple CTEs", () => {
    validateIdentity(
      "WITH a AS (SELECT 1), b AS (SELECT 2) SELECT * FROM a",
    );
  });

  it("CTE with UNION", () => {
    validateIdentity(
      "WITH cte AS (SELECT 1) SELECT * FROM cte UNION ALL SELECT 2",
    );
  });
});

// =============================================================================
// ClickHouse CASE WHEN
// =============================================================================

describe("ClickHouse: CASE WHEN", () => {
  it("simple CASE WHEN", () => {
    validateIdentity(
      "SELECT CASE WHEN a > 1 THEN 'yes' ELSE 'no' END FROM test",
    );
  });

  it("CASE with operand", () => {
    validateIdentity("SELECT CASE a WHEN 1 THEN 'one' WHEN 2 THEN 'two' ELSE 'other' END");
  });
});

// =============================================================================
// ClickHouse CAST with type mappings
// =============================================================================

describe("ClickHouse: CAST type mappings", () => {
  it("BOOLEAN -> Bool", () => {
    validateTranspile("SELECT CAST(a AS BOOLEAN)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Bool)",
    });
  });

  it("TINYINT -> Int8", () => {
    validateTranspile("SELECT CAST(a AS TINYINT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Int8)",
    });
  });

  it("SMALLINT -> Int16", () => {
    validateTranspile("SELECT CAST(a AS SMALLINT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Int16)",
    });
  });

  it("INT -> Int32", () => {
    validateTranspile("SELECT CAST(a AS INT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Int32)",
    });
  });

  it("BIGINT -> Int64", () => {
    validateTranspile("SELECT CAST(a AS BIGINT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Int64)",
    });
  });

  it("FLOAT -> Float32", () => {
    validateTranspile("SELECT CAST(a AS FLOAT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Float32)",
    });
  });

  it("DOUBLE -> Float64", () => {
    validateTranspile("SELECT CAST(a AS DOUBLE)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Float64)",
    });
  });

  it("VARCHAR -> String", () => {
    validateTranspile("SELECT CAST(a AS VARCHAR)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS String)",
    });
  });

  it("TEXT -> String", () => {
    validateTranspile("SELECT CAST(a AS TEXT)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS String)",
    });
  });

  it("CHAR -> String", () => {
    validateTranspile("SELECT CAST(a AS CHAR)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS String)",
    });
  });

  it("BINARY -> String", () => {
    validateTranspile("SELECT CAST(a AS BINARY)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS String)",
    });
  });

  it("BLOB -> String", () => {
    validateTranspile("SELECT CAST(a AS BLOB)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS String)",
    });
  });

  it("DATETIME -> DateTime", () => {
    validateTranspile("SELECT CAST(a AS DATETIME)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS DateTime)",
    });
  });

  it("TIMESTAMP -> DateTime", () => {
    validateTranspile("SELECT CAST(a AS TIMESTAMP)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS DateTime)",
    });
  });

  it("DECIMAL -> Decimal", () => {
    validateTranspile("SELECT CAST(a AS DECIMAL)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Decimal)",
    });
  });

  it("DECIMAL with precision preserved", () => {
    validateTranspile("SELECT CAST(a AS DECIMAL(18, 2))", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Decimal(18, 2))",
    });
  });

  it("DATE -> Date", () => {
    validateTranspile("SELECT CAST(a AS DATE)", {
      write: "clickhouse",
      expected: "SELECT CAST(a AS Date)",
    });
  });
});

// =============================================================================
// ClickHouse: ILIKE support
// =============================================================================

describe("ClickHouse: ILIKE support", () => {
  it("ILIKE round-trips natively", () => {
    validateIdentity("SELECT a FROM test WHERE a ILIKE '%foo%'");
  });

  it("ILIKE from default to ClickHouse preserves ILIKE", () => {
    validateTranspile("SELECT a FROM t WHERE a ILIKE '%test%'", {
      read: "",
      write: "clickhouse",
      expected: "SELECT a FROM t WHERE a ILIKE '%test%'",
    });
  });

  it("ILIKE from ClickHouse to MySQL becomes LIKE", () => {
    validateTranspile("SELECT a FROM t WHERE a ILIKE '%test%'", {
      read: "clickhouse",
      write: "mysql",
      expected: "SELECT a FROM t WHERE a LIKE '%test%'",
    });
  });

  it("ILIKE from ClickHouse to BigQuery becomes LIKE", () => {
    validateTranspile("SELECT a FROM t WHERE a ILIKE '%test%'", {
      read: "clickhouse",
      write: "bigquery",
      expected: "SELECT a FROM t WHERE a LIKE '%test%'",
    });
  });

  it("ILIKE from ClickHouse to SQLite becomes LIKE", () => {
    validateTranspile("SELECT a FROM t WHERE a ILIKE '%test%'", {
      read: "clickhouse",
      write: "sqlite",
      expected: "SELECT a FROM t WHERE a LIKE '%test%'",
    });
  });
});

// =============================================================================
// ClickHouse Aggregate Functions
// =============================================================================

describe("ClickHouse: aggregate functions", () => {
  it("COUNT(*)", () => {
    validateIdentity("SELECT COUNT(*) FROM test");
  });

  it("SUM", () => {
    validateIdentity("SELECT SUM(a) FROM test");
  });

  it("AVG", () => {
    validateIdentity("SELECT AVG(a) FROM test");
  });

  it("MIN", () => {
    validateIdentity("SELECT MIN(a) FROM test");
  });

  it("MAX", () => {
    validateIdentity("SELECT MAX(a) FROM test");
  });

  it("COALESCE", () => {
    validateIdentity("SELECT COALESCE(a, b, c) FROM test");
  });

  it("NULLIF", () => {
    validateIdentity("SELECT NULLIF(a, b) FROM test");
  });
});

// =============================================================================
// ClickHouse Window Functions
// =============================================================================

describe("ClickHouse: window functions", () => {
  it("ROW_NUMBER", () => {
    validateIdentity("SELECT ROW_NUMBER() OVER (ORDER BY a) FROM test");
  });

  it("RANK with PARTITION BY", () => {
    validateIdentity(
      "SELECT RANK() OVER (PARTITION BY a ORDER BY b) FROM test",
    );
  });

  it("SUM window", () => {
    validateIdentity("SELECT SUM(a) OVER (PARTITION BY b) FROM test");
  });
});

// =============================================================================
// ClickHouse: Subqueries
// =============================================================================

describe("ClickHouse: subqueries", () => {
  it("subquery in FROM", () => {
    validateIdentity("SELECT a FROM (SELECT a FROM test) AS t");
  });

  it("subquery in WHERE", () => {
    validateIdentity(
      "SELECT a FROM test WHERE a > (SELECT MAX(b) FROM other)",
    );
  });

  it("subquery in IN", () => {
    validateIdentity("SELECT a FROM test WHERE a IN (SELECT b FROM other)");
  });
});

// =============================================================================
// ClickHouse: String and numeric literals
// =============================================================================

describe("ClickHouse: string and numeric literals", () => {
  it("string literal", () => {
    validateIdentity("SELECT 'hello'");
  });

  it("escaped quote in string", () => {
    validateIdentity("SELECT ''''");
  });

  it("numeric literal", () => {
    validateIdentity("SELECT 42");
  });

  it("float literal", () => {
    validateIdentity("SELECT 3.14");
  });

  it("NULL literal", () => {
    validateIdentity("SELECT NULL");
  });

  it("TRUE literal", () => {
    validateIdentity("SELECT TRUE");
  });

  it("FALSE literal", () => {
    validateIdentity("SELECT FALSE");
  });
});

// =============================================================================
// ClickHouse: Complex queries
// =============================================================================

describe("ClickHouse: complex queries", () => {
  it("complex SELECT with WHERE, GROUP BY, HAVING, ORDER BY", () => {
    validateIdentity(
      "SELECT a, COUNT(*) FROM test WHERE b > 1 GROUP BY a HAVING COUNT(*) > 2 ORDER BY a",
    );
  });

  it("nested subquery", () => {
    validateIdentity(
      "SELECT a FROM (SELECT a FROM (SELECT a FROM test) AS t1) AS t2",
    );
  });

  it("CTE with UNION", () => {
    validateIdentity(
      "WITH cte AS (SELECT 1) SELECT * FROM cte UNION ALL SELECT 2",
    );
  });

  it("multiple JOINs with conditions", () => {
    validateIdentity(
      "SELECT a.x, b.y FROM a JOIN b ON a.id = b.id JOIN c ON b.id = c.id WHERE a.x > 1",
    );
  });

  it("CASE in SELECT with backtick-quoted columns", () => {
    validateIdentity(
      "SELECT CASE WHEN `status` = 1 THEN 'active' ELSE 'inactive' END AS `result` FROM `users`",
    );
  });
});

// =============================================================================
// ClickHouse: NOT variations
// =============================================================================

describe("ClickHouse: NOT variations", () => {
  it("NOT IN (parser generates NOT x IN form)", () => {
    validateIdentity(
      "SELECT a FROM test WHERE a NOT IN (1, 2, 3)",
      "SELECT a FROM test WHERE NOT a IN (1, 2, 3)",
    );
  });

  it("NOT BETWEEN (parser generates NOT x BETWEEN form)", () => {
    validateIdentity(
      "SELECT a FROM test WHERE a NOT BETWEEN 1 AND 10",
      "SELECT a FROM test WHERE NOT a BETWEEN 1 AND 10",
    );
  });

  it("NOT LIKE (parser generates NOT x LIKE form)", () => {
    validateIdentity(
      "SELECT a FROM test WHERE a NOT LIKE '%test%'",
      "SELECT a FROM test WHERE NOT a LIKE '%test%'",
    );
  });
});

// =============================================================================
// ClickHouse: Comment handling
// =============================================================================

describe("ClickHouse: comment handling", () => {
  it("single line comment with --", () => {
    const result = transpile("SELECT 1 -- comment", {
      readDialect: "clickhouse",
      writeDialect: "clickhouse",
    })[0];
    expect(result).toBe("SELECT 1");
  });

  it("block comment /* */", () => {
    const result = transpile("SELECT /* comment */ 1", {
      readDialect: "clickhouse",
      writeDialect: "clickhouse",
    })[0];
    expect(result).toBe("SELECT 1");
  });
});

// =============================================================================
// ClickHouse: NULL and BOOLEAN handling
// =============================================================================

describe("ClickHouse: NULL and BOOLEAN", () => {
  it("NULL literal", () => {
    validateIdentity("SELECT NULL");
  });

  it("TRUE literal", () => {
    validateIdentity("SELECT TRUE");
  });

  it("FALSE literal", () => {
    validateIdentity("SELECT FALSE");
  });

  it("IS NULL", () => {
    validateIdentity("SELECT a IS NULL FROM test");
  });

  it("IS TRUE", () => {
    validateIdentity("SELECT a IS TRUE FROM test");
  });

  it("IS FALSE", () => {
    validateIdentity("SELECT a IS FALSE FROM test");
  });
});

// =============================================================================
// ClickHouse: Cross-dialect type transpilation
// =============================================================================

describe("ClickHouse: cross-dialect type transpilation", () => {
  it("CAST from default to ClickHouse maps INT -> Int32", () => {
    validateTranspile("SELECT CAST(x AS INT)", {
      read: "",
      write: "clickhouse",
      expected: "SELECT CAST(x AS Int32)",
    });
  });

  it("CAST from default to ClickHouse maps DOUBLE -> Float64", () => {
    validateTranspile("SELECT CAST(x AS DOUBLE)", {
      read: "",
      write: "clickhouse",
      expected: "SELECT CAST(x AS Float64)",
    });
  });

  it("CAST from default to ClickHouse maps VARCHAR -> String", () => {
    validateTranspile("SELECT CAST(x AS VARCHAR)", {
      read: "",
      write: "clickhouse",
      expected: "SELECT CAST(x AS String)",
    });
  });
});

// =============================================================================
// ClickHouse: Features not yet implemented (marked as todo)
// =============================================================================

describe("ClickHouse: features not yet implemented", () => {
  it.todo("Array type and arrayJoin");
  it.todo("Tuple type");
  it.todo("Map type");
  it.todo("Nullable(T) wrapper type");
  it.todo("LowCardinality(T) wrapper type");
  it.todo("Nested type");
  it.todo("Enum8 / Enum16 types");
  it.todo("FixedString(N) type");
  it.todo("CREATE TABLE with MergeTree engines");
  it.todo("ALTER TABLE / DROP TABLE");
  it.todo("INSERT INTO ... VALUES / FORMAT");
  it.todo("FINAL keyword after table name");
  it.todo("PREWHERE clause");
  it.todo("GLOBAL IN / GLOBAL JOIN");
  it.todo("SAMPLE clause");
  it.todo("FORMAT clause (JSON, CSV, etc.)");
  it.todo("Lambda expressions (x -> x + 1)");
  it.todo("arrayMap, arrayFilter, arrayReduce");
  it.todo("toDateTime, toDate, toInt32 etc.");
  it.todo("MATERIALIZED / ALIAS columns in CREATE TABLE");
  it.todo("WITH TOTALS in GROUP BY");
  it.todo("SETTINGS clause");
  it.todo("multiIf function");
  it.todo("Geo types (Point, Ring, Polygon, etc.)");
  it.todo("IPv4 / IPv6 types");
});
